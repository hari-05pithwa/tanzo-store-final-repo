...
import Image from "next/image";
import Link from "next/link";
import PageTransition from "./PageTransition"; // Assuming it's in the same folder
    ...
    <PageTransition>
      <section className="relative w-full min-h-screen flex flex-col overflow-hidden bg-[#0A0A0A]">
        ...
        <motion.div 
          ...
          initial="initial"
          animate="animate"
          className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 pt-20"
        >
          {/* Floating Tag */}
          <motion.div variants={itemVars} className="mb-8 flex items-center gap-3 py-2 px-5 border border-white/20 rounded-full bg-white/5 backdrop-blur-md">
            <span className="relative flex h-1.5 w-1.5">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
              <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-white"></span>
            </span>
          ...
          {/* Cinematic Heading */}
          <motion.h1 variants={itemVars} className="text-7xl md:text-[130px] font-extralight leading-[0.8] text-white tracking-tighter text-center mb-8">
            Define <br /> 
            <span className="italic font-serif opacity-90 block mt-4">Your</span> 
            Base<span className="text-white/30 ml-1">.</span>
          </motion.h1>

          <motion.p variants={itemVars} className="text-gray-400 text-[10px] md:text-xs font-light tracking-[0.4em] max-w-[350px] text-center leading-relaxed uppercase">
            Curated garments for the modern minimalist.
          </motion.p>
        </motion.div>
