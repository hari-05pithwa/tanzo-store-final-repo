...
//     // This check will now pass because status is "Pending"
//     if (order.status !== "Pending") {
//       return NextResponse.json({ error: "Cannot cancel an order that is already processed" }, { status: 400 });
//     }
...
export async function DELETE(req, { params }) {
  try {
    ...
    // Find and delete the order specifically belonging to this user
    ...
    return NextResponse.json({ success: true, message: "Order removed from Atlas" }, { status: 200 });
  } catch (error) {
    console.error("Delete API Error:", error);
