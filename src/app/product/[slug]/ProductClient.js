...
//         action: {
//           label: "Login",
//           onClick: () => router.push("/auth/sign-in"), // Adjust path to your login page
//         },
//       });
//       return;
//     }
          ...
          >
            <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#F9F9F9]">
              ...
              <motion.div 
                ...
                className="absolute top-6 left-6 bg-black text-white px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em]"
              >
                Edition 2025
              </motion.div>
            </div>
          ...
          >
            <motion.div variants={fadeUp} className="mb-10">
              <span className="text-[10px] font-bold uppercase tracking-[0.5em] text-blue-600 mb-4 block">
                ...
                <span className="font-serif italic text-gray-400 lowercase font-light tracking-normal ml-1">Archive</span>
              </span>
              <h1 className="text-4xl xl:text-5xl font-light tracking-tighter text-gray-900 mb-4 uppercase leading-none">
                {product.name}
              </h1>
            ...
            <motion.div variants={fadeUp} className="space-y-8 border-y border-gray-100 py-10">
              <div>
                <h4 className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3 italic">Description</h4>
                <p className="text-md leading-relaxed text-gray-700 font-light italic">
                  "{product.description}"
                </p>
              </div>
              <div className="flex gap-10 text-[11px] uppercase tracking-widest">
              ...
              <div className="grid grid-cols-4 gap-2">
                {product.sizes?.map((size) => (
                  <motion.button
                    ...
                    key={size}
                    onClick={() => setSelectedSize(size)}
                    className={`h-14 flex items-center justify-center text-[12px] font-bold transition-all duration-300 ${
                      selectedSize === size ? "bg-black text-white" : "bg-white text-gray-900 border border-gray-100 hover:border-black"
                    }`}
                  >
                    {size}
                  </motion.button>
            ...
            <motion.div variants={fadeUp} className="mt-10 flex flex-col gap-4">
              <div className="flex items-center gap-4">
                <div className="flex items-center border border-gray-200">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-12 h-14 hover:bg-gray-50">-</button>
                  <span className="w-12 h-14 flex items-center justify-center font-mono">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="w-12 h-14 hover:bg-gray-50">+</button>
                </div>
                
                <button
                  onClick={handleAddToCart}
                  className="group relative flex-1 h-14 bg-black text-white text-[11px] font-bold uppercase tracking-[0.3em] overflow-hidden"
                >
                  <span className="relative z-10">Add to Bag — {selectedSize}</span>
                  <motion.div className="absolute inset-0 bg-zinc-800 translate-y-full group-hover:translate-y-0 transition-transform duration-500" />
                </button>
              </div>
            </motion.div>

            <motion.div variants={fadeUp} className="mt-12 flex flex-col gap-4 border-t border-gray-50 pt-8">
              {['Complimentary Shipping on all orders', '14-Day Returns Policy'].map((text, i) => (
                <div key={i} className="flex items-center gap-4 text-[10px] uppercase tracking-widest text-gray-400">
                  <motion.div 
                    initial={{ width: 0 }} 
                    animate={{ width: 32 }} 
